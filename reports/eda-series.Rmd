...
```{r}
# talvez fosse mais fácil visualizar a comparação das séries deixando todas num mesmo gráfico. ótimo relatório!

escolhidas %>% 
    filter(series_name == c("American Horror Story", "Breaking Bad", "Game of Thrones")) %>% 
    mutate(Série = series_name) %>% 
    ggplot(aes(x = series_ep,
               y = user_rating, 
               color = series_name)) +
    ...
    xlab("Episódio") +
    ylab("Avaliação dos usuários") +
    ggtitle("Avaliações dos usários para cada episódio")
```
